...
public class RouteController : ControllerBase
{
    ...
    public RouteController(IMongoClient client, IConfiguration config)
    {
    ...
    {
        if (string.IsNullOrEmpty(_apiKey)) return BadRequest("Google API Key is missing.");

        // --- QUOTA SAVING: CHECK CACHE IN MONGODB ---
        // එකම Start සහ End සඳහා කලින් සෙවුමක් කර ඇත්නම් එය නැවත පාවිච්චි කරයි
        ...
        {
            return Ok(existingRoute); // Google API එක Call නොකර පරණ දත්ත ලබා දෙයි
        }
        ...
        try
        {
            ...
            var fastestBody = new
            {
                origin = new { address = req.Start },
                destination = new { address = req.End },
                travelMode = "DRIVE",
                routingPreference = "TRAFFIC_AWARE_OPTIMAL"
            };

            // --- 2. CHEAPEST ROUTE ---
            var cheapestBody = new
            {
                origin = new { address = req.Start },
                destination = new { address = req.End },
                travelMode = "DRIVE",
            ...
            {
                return NotFound(new { message = "Routes not found.", debug = fastestRaw });
            }

            var fRoute = fRoutes[0];
            JsonElement cRoute = (cheapestDoc.RootElement.TryGetProperty("routes", out JsonElement cRoutes) && cRoutes.GetArrayLength() > 0)
                                 ? cRoutes[0] : fRoute;

            JsonElement sRoute = (scenicDoc.RootElement.TryGetProperty("routes", out JsonElement sRoutes) && sRoutes.GetArrayLength() > 1)
                                 ? sRoutes[1] : (sRoutes.GetArrayLength() > 0 ? sRoutes[0] : fRoute);
            ...
            {
                Console.WriteLine("Scenic Viewpoints Error: " + ex.Message);
            }
            ...
            var newRoute = new SavedRoute
            {
                ...
                Fastest = new RouteDetail
                {
                    ...
                    Duration = fRoute.GetProperty("duration").GetString(),
                ...
                Cheapest = new RouteDetail
                {
                    ...
                    Duration = cRoute.GetProperty("duration").GetString(),
                ...
                Scenic = new RouteDetail
                {
                    ...
                    Duration = sRoute.GetProperty("duration").GetString(),
